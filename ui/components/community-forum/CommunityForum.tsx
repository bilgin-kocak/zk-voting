            ...
            <Accordion.Root type="single" collapsible>
                {['general', 'policies', 'support'].map((category) => (
                    <Accordion.Item key={category} value={category}>
                        <Accordion.Header>
                            <Accordion.Trigger>{category.charAt(0).toUpperCase() + category.slice(1)} Discussions</Accordion.Trigger>
                        </Accordion.Header>
                        <Accordion.Content>
                            ...
                            <ul>
                                {posts.filter(post => post.category === category).map(post => (
                                    <li key={post._id}>
                                        <h3>{post.title}</h3>
                                        <p>{post.content}</p>
                                        <small>Posted by {post.createdBy} on {new Date(post.createdAt).toLocaleDateString()}</small>
                                    </li>
                                ))}
                            </ul>
                        </Accordion.Content>
