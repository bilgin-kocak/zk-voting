...
displayTree(offChainInstance.voteCountMerkleTree);
{
  ...
  const votersWitness = new VoterListMerkleWitness(
    offChainInstance.votersMerkleTree.getWitness(0n)
  );
...

{
  ...
  const votersWitness = new VoterListMerkleWitness(
    offChainInstance.votersMerkleTree.getWitness(1n)
  );
  const votingID = zkAppInstance.votingID.get();
...

{
  ...
  // create witness for index 1
  const votersWitness = new VoterListMerkleWitness(
    offChainInstance.votersMerkleTree.getWitness(3n)
  );
  const votingID = zkAppInstance.votingID.get();
...

{
  ...
  const offChainStateInstance = new offChainStateChangeStruct({
    ...
    modifiedNullifierMapMerkleRoot:
      offChainInstance.nullifierMerkleMap.getRoot(),
