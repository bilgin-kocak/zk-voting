  ...
  methods: {
    verifyData: {
      ...
      method(
        ...
        leaf.assertEquals(
          Poseidon.hash(voterPrivateKey.toPublicKey().toFields())
        );
        ...
        const calculatedNullifierHash = Poseidon.hash(
          voterPrivateKey.toFields().concat([voterInfo.votingID])
        );
        calculatedNullifierHash.assertEquals(
          Poseidon.hash(voterPrivateKey.toFields().concat([voterInfo.votingID]))
        );
...
export const offChainStateProofs = ZkProgram({
  ...
  methods: {
    vote: {
      ...
      method(
        stateChange: offChainStateChangeStruct,
        voteProof: Proof<votingDataStruct, void>,
        ...
        voteProof.publicInput.votersMerkleTreeRoot.assertEquals(
          stateChange.votersMerkleRoot
        );
        ...
        stateChange.nullifierMerkleRoot.assertEquals(
          calculatedNullifierMerkleRoot
        );
        calculatedNullifierKey.assertEquals(
          voteProof.publicInput.nullifierHash
        );

        // Check the previous vote count is correct
        const calculatedVoteCountRoot =
          voteCountWitness.calculateRoot(voteCountLeaf);
        stateChange.voteCountMerkleRoot.assertEquals(calculatedVoteCountRoot);

        // Do voting
        stateChange.modifiedVoteCountMerkleRoot.assertEquals(
          voteCountWitness.calculateRoot(voteCountLeaf.add(1))
        );
        ...
        stateChange.modifiedNullifierMapMerkleRoot.assertEquals(
          newNullifierMerkleRoot
        );
      },
    },
    merge: {
      ...
      method(
        stateChange: offChainStateChangeStruct,
        proof1: SelfProof<offChainStateChangeStruct, offChainStateChangeStruct>,
        proof2: SelfProof<offChainStateChangeStruct, offChainStateChangeStruct>
      ) {
        // Verify both the proofs
        proof1.verify();
        proof2.verify();

        stateChange.votersMerkleRoot.assertEquals(
          proof1.publicInput.votersMerkleRoot
        );
        stateChange.votersMerkleRoot.assertEquals(
          proof2.publicInput.votersMerkleRoot
        );
        ...
        stateChange.nullifierMerkleRoot.assertEquals(
          proof1.publicInput.nullifierMerkleRoot
        );
        stateChange.voteCountMerkleRoot.assertEquals(
          proof1.publicInput.voteCountMerkleRoot
        );

        proof1.publicInput.modifiedNullifierMapMerkleRoot.assertEquals(
          proof2.publicInput.nullifierMerkleRoot
        );
        proof1.publicInput.modifiedVoteCountMerkleRoot.assertEquals(
          proof2.publicInput.voteCountMerkleRoot
        );

        proof2.publicInput.modifiedNullifierMapMerkleRoot.assertEquals(
          stateChange.modifiedNullifierMapMerkleRoot
        );
        proof2.publicInput.modifiedVoteCountMerkleRoot.assertEquals(
          stateChange.modifiedVoteCountMerkleRoot
        );
      },
    },
  },
